...
var ennemi5;
var vieEnnemi5 = 12;
var ennemi5Mort = false;
var ennemi5Invinsible = false;
var ennemi5Stop = false;
var ennemi5Aggro = false;

class pc_TB2 extends Phaser.Scene{
    constructor(){
        super("pc_TB2");
    }
    init(data){
    ...
    preload ()
    {
        ...
        this.load.tilemapTiledJSON('map8','assets/placeholder/pc_TB2.json');
    ...
    create ()
    {
        ...
        if (position == "TB1-TB2"){
            player = this.physics.add.sprite(100, 2100, 'dude');
        }
        else if (position == "TB3-TB2"){
        ...
        if (vieEnnemi5 > 0 ){
            ennemi5 = ennemis.create(1401, 1300, 'ennemi');
            ennemi5.body.height = 100;
            ennemi5.body.setOffset(0, 50);  
        }
       

        this.physics.add.collider(orange, ennemi5);
        this.physics.add.collider(bleu, ennemi5);

        this.physics.add.overlap(ennemi5, epees, epeeEnnemi);
        this.physics.add.overlap(ennemi5, boulets, bouletEnnemi);
        this.physics.add.overlap(ennemi5, explosions, explosionEnnemi);
        this.physics.add.overlap(ennemi5, megaExplosions, megaExplosionEnnemi);

        this.physics.add.overlap(player, ennemis, hitEnnemi);

        function epeeEnnemi(ennemi5, epees){
            if (ennemiInvinsible == false){
                vieEnnemi5 -= 3;
                if(vieEnnemi5 <= 0){
                    potion = potions.create(ennemi5.x, ennemi5.y, 'potion');
                    ennemi5.destroy();
                    ennemi5Mort = true;
                }
                else{
                    ennemi5Invinsible=true;
                    ennemi5Stop = true;
                    ennemi5.setVelocityX(0);
                    ennemi5.setVelocityY(0);
                    ennemi5.alpha = 0.5;
                    setTimeout(function(){ennemi5Invinsible = false}, 1000);
                    setTimeout(function(){ennemi5.alpha = 1}, 1000);
                    setTimeout(function(){ennemi5Stop=false}, 1000);  
                }
            }
        ...
        function megaExplosionEnnemi(ennemi5, megaExplosions){
            if (ennemi5Invinsible == false){
                vieEnnemi5 -= 8;
                if(vieEnnemi5 <= 0){
                    potion = potions.create(ennemi5.x, ennemi5.y, 'potion');
                    ennemi5.destroy();
        ...
        function hitEnnemi(player, ennemi5){
            if (invinsible == false){
                vie -= 1;
                invinsible = true;
                player.alpha = 0.5;
        ...
        tonneau = tonneaux.create(650, 150, 'tonneau');

        tonneau = tonneaux.create(1750, 1950, 'tonneau');
        ...
        caisse = caisses.create(950, 850, 'caisse');
        caisse = caisses.create(1050, 850, 'caisse');
        ...
        barricade = barricades.create(300, 1100, 'barricade');

        barricade = barricades.create(1500, 1000, 'barricade');
        barricade = barricades.create(1700, 1000, 'barricade');
        ...
        barril = barrils.create(950, 1450, 'barril');
        barril = barrils.create(950, 2050, 'barril');
        ...
        tonneauPotion = tonneauxPotion.create(1850, 150, 'tonneau');
    ...
    update ()
    {
        ...
        if (ennemi5.x < 400 && ennemi5.y == 1300 && ennemi5Stop == false && ennemi5Mort == false){
            ennemi5.setVelocityX(250);
            ennemi5.setVelocityY(0);
        }
        if (ennemi5.x > 1400 && ennemi5.y == 1300 && ennemi5Stop == false && ennemi5Mort == false){
            ennemi5.setVelocityX(-250);
            ennemi5.setVelocityY(0);
        }
        if(ennemi5.y == 1300 && 600<ennemi5.x>1400 && ennemi5Stop == false && ennemi5Mort == false){
        ...
        if (Math.pow((Math.pow(player.x-ennemi5.x,2))+(Math.pow(player.y-ennemi5.y,2)),1/2)<=300){
            ennemi5Aggro = true;
        }
        if (ennemi5Aggro == true && ennemi5Stop == false && ennemi5Mort == false){
            ennemi5.setVelocityX(player.x-ennemi5.x);
            ennemi5.setVelocityY(player.y-ennemi5.y);
            ennemi5.setMaxVelocity(250);
        }
